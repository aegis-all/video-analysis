...
OpenCV で動画を1コマずつ調べるので、**メモリと CPU を使います**。
Starter プランはどちらも控えめなので、

- 生成が **この PC より遅くなります**（60秒の動画で 20秒 → 40〜60秒の見込み）
- 縦長の高解像度や長い動画で、**メモリが足りずに落ちる可能性があります**

まず Starter で試して、落ちるようなら次のどちらかにしてください。

1. メモリの大きいプランへ上げる（月額が上がります）
2. スクショ生成だけこの PC で行い、できたものを Render へ移す

打ち切り時間は `VR_JOB_TIMEOUT`（既定 1800秒）で変えられます。

> 費用を抑えたい場合、**VPS（[DEPLOY.md](DEPLOY.md)、年 11,000円ほど）なら
> メモリ 2GB が最初から付きます。**手間はかかりますが、
> 動画を残す運用ではこちらのほうが安く済みます。
